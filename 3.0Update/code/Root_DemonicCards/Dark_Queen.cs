    ...
    public class Dark_Queen: MonoBehaviour {
	    ...
	    public void Telleport() {
	        if(timer<=0) {
	            ...
	            if(Physics2D.OverlapCircle(vector, 0.5f)||!player.data.view.IsMine)
	                return;
	            player.GetComponentInParent<PlayerCollision>().IgnoreWallForFrames(2);
	            player.transform.position=vector;
	            Core.instance.ExecuteAfterFrames(2, () => player.data.healthHandler.CallTakeDamage(Vector2.up*player.data.maxHealth*0.34f, vector, null, player, true));
